...
export function verifyInjectedProvider(check: string): boolean {
  ...
  return anyWindow.ethereum
    ? anyWindow.ethereum[check]
    : anyWindow.web3 &&
        anyWindow.web3.currentProvider &&
        anyWindow.web3.currentProvider[check];
}
...
export function findMatchingRequiredOptions(
  ...
  return requiredOptions.filter((requiredOption) => {
